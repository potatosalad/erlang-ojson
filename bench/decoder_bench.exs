defmodule DecoderBench do
  ...
  # UTF8 escaping
  bench "UTF-8 unescaping (OJSON)", [utf8: gen_utf8()] do
    OJSON.decode!(utf8)
  end
  ...
  bench "Issue 90 (OJSON)", [json: gen_issue90()] do
    OJSON.decode!(json)
  end
  ...
  bench "Issue 90 (jsone)", [json: gen_issue90()] do
    :jsone.decode(json)
  end

  defp gen_utf8() do
    text = File.read!(Path.expand("data/UTF-8-demo.txt", __DIR__))
    OJSON.encode!(text) |> IO.iodata_to_binary()
  end
